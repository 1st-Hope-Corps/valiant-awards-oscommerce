...
if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'process')) {
	...
	if (!tep_db_num_rows($check_customer_query)) {
		...
		//echo '<pre>';print_r($check_customer);exit;
		...
		if (!tep_validate_password($password, $check_customer['customers_password'])) {
			...
			if(!$iMerchantId){$iMerchantId=$HTTP_POST_VARS['uid'];}
			//echo $HTTP_POST_VARS['uid'];
			//echo '<pre>';print_r($iMerchantId);exit;
			...
			if ($sStoreType == "store"){
				tep_redirect("index2.php?language=en&merchant_id=".$iMerchantId);
			}elseif ($sStoreType == "comm"){
				tep_redirect("index2.php?language=en&merchant_id=1");
...
<!-- body_text //-->
<!-- FILENAME_LOGIN -->
    <td width="100%" class="col_center"><?php echo tep_draw_form('login2', tep_href_link('login2.php', 'action=process', 'SSL')); ?><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        <td>
