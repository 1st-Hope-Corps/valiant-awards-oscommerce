...
<div id="mobile-container">
  ...
  <div id="mobile-header-container">
    ...
    <div id="mobile-header-title-container"  style="width: 45%;text-align: center;">
      <h2 id="mobile-header-title"><a href="/index.php">Checkout Confirmation</a></h2>
    </div>
  ...
  <div id="mobile-content">
    <!-- <div class="h2" style="text-align:center;">Checkout Confirmation</div> -->
    <form action="/checkout_process.php" method="POST">
      <div style="text-align: center;">
        <div class="container-information">
          <div style="margin-bottom:15px"><b style="font-size:1.1rem">Products</b></div>
          <div style="width: 100%;margin-bottom: 5px;">
            <div style="float: left;width: 35%;font-weight: bold;">Name</div>
            <div style="float: left;width: 35%;font-weight: bold;">Qty</div>
            <div style="float: left;width: 30%;font-weight: bold;">Total</div>
            <div style="clear: both;"></div>
          </div>
          <div style="margin-top: 5px;">
            <?php for ($i=0, $n=sizeof($order->products); $i<$n; $i++) { ?>
            <div style="width: 100%;">
              <div style="float: left;width: 35%;"><?= $order->products[$i]['name'] ?></div>
              <div style="float: left;width: 35%;"><?= $order->products[$i]['qty'] ?></div>
              <div style="float: left;width: 30%;"><?= $currencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) ?></div>
              <div style="clear: both;"></div>
            </div>
            <? } ?>
            <hr>
            <div style="width: 100%;text-align: right;">
              <div style="float: left;width: 45%;font-weight: bold;">Total</div>
              <div style="float: left;width: 35%;"><?= $order_total_modules->output_mobile()['total']; ?></div>
              <div style="clear: both;"></div>
            </div>
          </div>
        </div>
        <div class="container-information">
          <div style="margin-bottom:15px"><b style="font-size:1.1rem">Billing Information</b></div>
          <div style="width: 100%;margin-bottom: 5px;">
            <div style="float: left;width: 40%;">Email Address</div>
            <div style="float: left;width: 60%;font-weight: bold;word-break: break-all;"><?= $_SESSION["email_address"] ?></div>
            <div style="clear: both;"></div>
          </div>
          <div style="width: 100%;margin-bottom: 5px;">
            <div style="float: left;width: 40%;">Payment Method</div>
            <div style="float: left;width: 60%;font-weight: bold;word-break: break-all;">Hope Vault</div>
            <div style="clear: both;"></div>
          </div>
        </div>
        <?php
        ...
        ?>
            <div class="container-information">
              <div style="margin-bottom:15px"><b style="font-size:1.1rem">Payment Information</b></div>
              <div style="width: 100%;margin-bottom: 5px;">
                <div style="float: left;width: 40%;">Hope Vault</div>
                <div style="float: left;width: 60%;font-weight: bold;word-break: break-all;"><?= $confirmation['fields'][0]['field'] ?></div>
                <div style="clear: both;"></div>
              </div>
            </div>
            <?php
...
for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
    ...
    if ( (isset($order->products[$i]['attributes'])) && (sizeof($order->products[$i]['attributes']) > 0) ) {
      for ($j=0, $n2=sizeof($order->products[$i]['attributes']); $j<$n2; $j++) {
		...
		if ($order->products[$i]['attributes'][$j]['option'] == "Download") $bDownload = true;
	  }
    }
	...
	if (!$bDownload && $_SESSION["sStoreType"] == "comm"){
		echo '<tr>
					<td class="main" colspan="3" style="padding:5px 0px 0px 40px;"><div>This product is available for pickup at the Hope Cybrary only. To continue, press "Confirm Order" button.</div></td>
				</tr>';
	}
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td width="30%" valign="top"><table border="0" width="100%" cellspacing="4" cellpadding="2">
              <tr>
                <td class="main"><?php echo '<b>' . HEADING_BILLING_ADDRESS_DOWNLOAD . '</b> <a href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT_ADDRESS, '', 'SSL') . '"><span class="orderEdit">(' . TEXT_EDIT . ')</span></a>'; ?></td>
              </tr>
              <tr>
                <td class="main"><?php echo $_SESSION["email_address"]; ?></td>
              </tr>
              <tr>
