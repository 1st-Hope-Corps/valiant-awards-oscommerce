...
if (isset($_POST["bSaveFromNing"]) && ($sHost == HOPE_WWW || $sHost == HOPE_DRUPAL || $sHost == "localhost")){
	...
	$sRefererURL = $_POST["sRefererURL"];
	$sRedirectTo = $_POST["sRedirectTo"];
	
	$iMaxSize = $_POST["MAX_FILE_SIZE"];
	$iImageSize = $_FILES["sProductImage"]["size"];
	$iVirtualSize = $_FILES["sProductToDownload"]["size"];
	
	if ($iImageSize <= 1024000){
		$bContinue = true;
	}else{
		$bErrorImage = true;
	}
	
	if ($sProductType == "download"){
		$bContinue = ($iVirtualSize <= 10240000);
		
		if ($iVirtualSize <= 10240000){
			$bContinue = true;
		}else{
			$bErrorVirtual = true;
		}
	...
	if ($bErrorImage && !$bErrorVirtual){
		$iError = 1;
	}elseif (!$bErrorImage && $bErrorVirtual){
		$iError = 2;
	}elseif ($bErrorImage && $bErrorVirtual){
		$iError = 3;
	}
	...
	if (!$bContinue){
		header("Location: ".$sRefererURL."?c=".$iCatID."&err=".$iError);
		?>
		<script type="text/javascript" language="JavaScript">
		top.location.href = "<?php echo $sRefererURL."?c=".$iCatID."&err=".$iError ?>";
		</script>
		<?php
	}
