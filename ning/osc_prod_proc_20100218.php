...
if (isset($_POST["bSaveFromNing"]) && ($sHost == HOPE_WWW || $sHost == HOPE_DRUPAL)){
	...
	if ($iMemberID != 1){
		...
		if ($_FILES["sProductToDownload"]["error"] == 0 && $_FILES["sProductToDownload"]["size"] > 0){
			...
			$sTempImageName = $_FILES["sProductToDownload"]["tmp_name"];
			$sItemName = time()."_".$iMemberID."_".$sItemName;
			$sItemPath = "../download/".$sItemName;
			
			if ($sItemExtName == "zip"){
				if (move_uploaded_file($sTempImageName, $sItemPath)) $bWithItem = true;
			}else{
				if (move_uploaded_file($sTempImageName, $sItemPath)){
					$bZipZuccess = CreateZip(array($sItemPath), substr($sItemPath, 0, strlen($sItemPath)-4).".zip");
