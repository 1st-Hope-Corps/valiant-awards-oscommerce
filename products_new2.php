...
<?php
 ...
 $product_query = tep_db_query("select products_description, products_id from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$products_new['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
     ...
     //FILENAME_PRODUCT_INFO
		$p_pic = '<a href="' . tep_href_link('product_info2.php', 'products_id=' . $products_new['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $products_new['products_image'], $products_new['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
		$p_name = '<a href="' . tep_href_link('product_info2.php', 'products_id=' . $products_new['products_id']) . '">' .$products_new['products_name'] . '</a>';
   ...
   if ($new_price = tep_get_products_special_price($products_new['products_id'])) {
    ...
    $info_box_contents[$row][$col] = array('align' => 'center',
			...
			<table cellpadding="0" cellspacing="0" border="0">
				...
				<tr><td align="right"><a href="' . tep_href_link('product_info2.php?products_id='.$p_id) . '">'.tep_image_button("button_details.gif").'</a><br style="line-height:1px;"><br style="line-height:5px;"><a href="'.tep_href_link("products_new.php","action=buy_now&products_id=".$p_id).'">'.tep_image_button('button_add_to_cart1.gif').'</a></td></tr>
