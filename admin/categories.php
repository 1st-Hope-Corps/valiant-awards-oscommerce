...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          if (isset($HTTP_POST_VARS['products_image_coupon']) && tep_not_null($HTTP_POST_VARS['products_image_coupon']) && ($HTTP_POST_VARS['products_image_coupon'] != 'none')) {
            $sql_data_array['products_image_coupon'] = tep_db_prepare_input($HTTP_POST_VARS['products_image_coupon']);
          }
...
// copy image only if modified
        ...
        $products_image_coupon = new upload('products_image_coupon');
        $products_image_coupon->set_destination(DIR_FS_CATALOG_IMAGES);
        if ($products_image->parse() && $products_image->save()) {
          $products_image_name = $products_image->filename;
          $products_image_coupon_name = $products_image_coupon->filename;
        } else {
          $products_image_name = (isset($HTTP_POST_VARS['products_previous_image']) ? $HTTP_POST_VARS['products_previous_image'] : '');
          $products_image_coupon_name = (isset($HTTP_POST_VARS['products_previous_image_coupon']) ? $HTTP_POST_VARS['products_previous_image_coupon'] : '');
        }

        if ($products_image_coupon->parse() && $products_image_coupon->save()) {
          $products_image_coupon_name = $products_image_coupon->filename;
        }else{
          $products_image_coupon_name = (isset($HTTP_POST_VARS['products_previous_image_coupon']) ? $HTTP_POST_VARS['products_previous_image_coupon'] : '');
        }
...
<?php
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_image_coupon, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
...
?>
          ...
          <tr>
            <td class="main"><?php echo TEXT_PRODUCTS_COUPON_IMAGE; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_coupon') . '<br>' . tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . $pInfo->products_image_coupon . tep_draw_hidden_field('products_previous_image_coupon', $pInfo->products_image_coupon); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
<?php
    ...
    if (tep_not_null($HTTP_POST_VARS)) {
      ...
      $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_image_coupon, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
      $product = tep_db_fetch_array($product_query);

      $pInfo = new objectInfo($product);
      $products_image_name = $pInfo->products_image;
      $products_image_coupon_name = $pInfo->products_image_coupon;
    }
...
?>
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td class="main"><?php echo tep_image(DIR_WS_CATALOG_IMAGES . $products_image_coupon_name, $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $pInfo->products_description; ?></td>
        <td class="main"><?php echo tep_image(DIR_WS_CATALOG_IMAGES . $products_image_name, 'Coupon', SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"'); ?> Coupon</td>
      </tr>
<?php
...
?>
      ...
      <tr>
        <td class="main" colspan="2"><?php echo sprintf(TEXT_PRODUCT_MORE_INFORMATION, $pInfo->products_url); ?></td>
      </tr>
<?php
      }
?>
...
?>
      <tr>
        <td align="center" class="smallText" colspan="2"><?php echo sprintf(TEXT_PRODUCT_DATE_AVAILABLE, tep_date_long($pInfo->products_date_available)); ?></td>
      </tr>
<?php
      } else {
?>
      <tr>
        <td align="center" class="smallText" colspan="2"><?php echo sprintf(TEXT_PRODUCT_DATE_ADDED, tep_date_long($pInfo->products_date_added)); ?></td>
      </tr>
<?php
      }
?>
...
?>
      <tr>
        <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link($back_url, $back_url_params, 'NONSSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
      </tr>
<?php
    } else {
?>
...
/* Re-Post all POST'ed variables */
      ...
      echo tep_draw_hidden_field('products_image_coupon', stripslashes($products_image_coupon_name));
